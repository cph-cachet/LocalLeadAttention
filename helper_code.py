...
def find_challenge_files(data_directory):
    ...
    header_files, recording_files = [x for x in all_files if x.endswith(
        '.hea')], [x for x in all_files if x.endswith('.mat')]

    uniqs = pd.value_counts(
        [x[:-4] for x in header_files] + [x[:-4] for x in recording_files])
    uniqs = list(uniqs.keys()[np.where(uniqs.values > 1)])

    return [x+'.hea' for x in uniqs], [x+'.mat' for x in uniqs]
